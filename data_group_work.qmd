...
# Dying all over the world: What changed over time and where does Portugal stand? {.main_header}
...
The theme that guided our work was *Dying all over the world: What changed over time and where does Portugal stand?* <br>
...
* What are the main risk factors for the main causes of death in Portugal?
...
#library(gganimate)   # Create dynamic ggplots 
#library(magick)     # Generate GIFs and image processing
...
```{r, testing, results='asis', include=TRUE, echo=FALSE}
#Show what are the levels for all factor variables
lv <- map(data %>% select(where(is.factor) & ends_with("name")), levels)
...
1) There are more rows with **Deaths** than **DALYs**, meaning there is some combination of variables for which no **Deaths** data is available.
...
In this section we explore how death causes have changed over time.
...
data %>%
  ...
  ggplot()+
  geom_col(aes(x = year, y = val, fill=fct_rev(fct_relevel(causes_group,"Other", after = Inf))), position = position_stack()) +
...
* Deaths by **Respiratory infections and tuberculosis** increased in Portugal by moire than 100%, while decreaing globally by 28.3%
...
```{r pt_percent_changes, include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
...
Any particularly relevant conclusion? If not, remove the table
...
### outliers are all on the top side, which makes sense, because "All happy families are alike, but every unhappy family is unhappy in its own way (Leo Tolstoy, Anna Karenina, 1878)", meaning that most countries with High SDI, which have lower mortality rates overall, share similar characteristics, "pulling" the mean down. on the other end of the spectrum, we may have a country wich a very specific combination of risk factors that causes it to have a death pattern from a specific cause that is very different from all others
